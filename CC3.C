...
int constexpr (int *val) {
  ...
  if (konst == 0) error ("must be constant expression");
  return konst;
}
...
void test (int label, int parens) {
  ...
  if (is[SA]) {             /* stage address of "oper 0" code */
    switch (is[OP]) {       /* operator code */
      ...
      case GE12u: clearstage ((int *) (is[SA]), 0);      break;	// XXX added cast
  ...
  if (ch == '[' || ch == '(') {
    ...
    while (1) {
      if (match ("[")) {              /* [subscript] */
        ...
        setstage ((int * *) (& before), (int * *) (& start)); 	// XXX added casts; unsure of setstage,stage declared correctly
        is2[TC] = 0;
        down2 (0, 0, level1, is2, is2);
        need ("]");
        if (is2[TC]) {
          clearstage ((int *) before, 0);	// XXX added cast; unsure if clearstage declared correctly
...
static int primary (int *is) {
  ...
  putint (0, (char *) is, 7 << LBPW);         /* clear "is" array */ // XXX added cast
...
static void callfunc (char *ptr) {      /* symbol table entry or 0 */
  int nargs, konst, val;
  nargs = 0;
  ...
  if (is[TC]) {                    /* consant op unknown */
    ...
    if (is2[TC]) {                 /* variable op constant */
      ...
      if (oper == ADD12) {         /* commutative */
        gen (GETw2n, is2[CV] << dbl (oper, is, is2));
        }
      else {                      /* non-commutative */
        gen (MOVE21, 0);
        gen (GETw1n, is2[CV] << dbl (oper, is, is2));
        }
      }
    else {                        /* variable op variable */
      gen (POP2, 0);
      if (dbl (oper, is, is2)) gen (DBL1, 0);
      if (dbl (oper, is2, is)) gen (DBL2, 0);
      }
    }
  if (oper) {
    ...
    if (is[ST] == 0 || ((ptr = (char *) (is2[ST])) && (ptr[TYPE] & UNSIGNED)))	// XXX added cast
      is[ST] = is2[ST];
    }
  }
...
static int nosign (int is[]) {
  ...
  if (is[TA]
  || is[TC] == UINT
  || ((ptr = (char *) (is[ST])) && (ptr[TYPE] & UNSIGNED))	// XXX cast added
    ) return 1;
  return 0;
  }
